...
impl RenderPass for ForwardPass {
    fn render(
        ...
        let mut render_pass = self.create_wgpu_render_pass(
            &mut encoder,
            &swap_texture,
            &frame_data.depth_view.as_ref().unwrap(),
        );
        ...
        for render_data in &mut frame_data.render_data_vec {
            ...
            unsafe {
                material_resource
                    .borrow_unsafe()
                    .set_uniforms(1, &mut render_pass, graphics)
            };
...
impl Renderer for DefaultRenderer {
    fn render(&mut self, frame: &mut Frame, world: &mut World) {
        ...
        if let Some(camera_entity) = camera_entity {
            ...
            // stats.set("culled_entities", Stat::usize(pre_cull_count - render_data_vec.len()), false);
            // stats.set("rendered_entities", Stat::usize(render_data_vec.len()), false);

            let mut frame_data = FrameData {
                depth_view: Some(
                    self.depth_texture.as_ref().unwrap().create_view(
            ...
            self.forward_pass
                .render(frame, &Graphics::global_read(), proj_view, &mut frame_data);
        } else {
